...
public struct TrialClock {

    /// Where the anchor is kept unless a caller names somewhere else.
    public static let defaultStartKey = "trial.startedAt"

    private let defaults: UserDefaults
    private let startKey: String

    public init(defaults: UserDefaults = .standard, startKey: String = TrialClock.defaultStartKey) {
    ...
    public func startDate(now: Date = Date()) -> Date {
        let stored = defaults.object(forKey: startKey) as? Date
    ...
    public var hasStarted: Bool {
        defaults.object(forKey: startKey) != nil
    }
    ...
    public func state(lengthInDays: Int, now: Date = Date()) -> TrialState {
    ...
    public func setStartDateForTesting(_ date: Date?) {
        if let date {
