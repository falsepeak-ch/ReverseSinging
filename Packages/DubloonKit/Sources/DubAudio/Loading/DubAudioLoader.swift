...
public enum DubAudioLoader {
    ...
    public static func loadBuffer(from url: URL) throws -> AVAudioPCMBuffer {
        ...
        // One read can stop short of the end of the file, so the rest is read in further passes
        // and appended. Otherwise the tail of a clip goes missing without anything saying so.
        while buffer.frameLength < buffer.frameCapacity, file.framePosition < file.length {
            guard let rest = AVAudioPCMBuffer(
                pcmFormat: format,
                frameCapacity: buffer.frameCapacity - buffer.frameLength
            ) else { break }

            try file.read(into: rest)
            guard rest.frameLength > 0, append(rest, to: buffer) else { break }
        }

        return buffer
    }

    /// Copies `source` onto the end of `destination`, when both carry float samples and it fits.
    private static func append(_ source: AVAudioPCMBuffer, to destination: AVAudioPCMBuffer) -> Bool {
        guard let from = source.floatChannelData,
              let to = destination.floatChannelData,
              source.format.channelCount == destination.format.channelCount,
              destination.frameLength + source.frameLength <= destination.frameCapacity else { return false }

        let offset = Int(destination.frameLength)
        for channel in 0..<Int(source.format.channelCount) {
            to[channel].advanced(by: offset).update(from: from[channel], count: Int(source.frameLength))
        }
    ...
    public static func loadVoiceBuffer(from url: URL, applyFades: Bool = true) throws -> AVAudioPCMBuffer {
    ...
    public static func convert(_ buffer: AVAudioPCMBuffer, to format: AVAudioFormat) throws -> AVAudioPCMBuffer {
        ...
        // The converter calls the input block synchronously, on this thread, before `convert`
        // returns; nothing else can see this flag.
    ...
    public static func applyEdgeFades(to buffer: AVAudioPCMBuffer) {
    ...
    public static func normalizeDuration(ofFileAt url: URL, to duration: TimeInterval) throws {
    ...
    public static func trimming(_ buffer: AVAudioPCMBuffer, fromOffset offset: TimeInterval) -> AVAudioPCMBuffer? {
