...
///
/// A live meter driven by the recorder's current level cannot show a file that already
/// exists. This is the other half: the shape of a
/// whole recording, so a take can be laid over the reference line and compared.
public actor WaveformSampler {

    public static let shared = WaveformSampler()
    ...
    public func samples(from url: URL, buckets: Int = 96) async -> [Float] {
...
public enum WaveformScaling {
    ...
    public static let maxOverrun = 3
    ...
    public static func bucketCount(
