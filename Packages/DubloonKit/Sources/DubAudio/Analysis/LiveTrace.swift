...
public struct LiveTrace: Sendable {
    ...
    public private(set) var loudest: Float = 0
    ...
    public var normalizedBars: [Float] {
        guard loudest > 0 else { return bars }
        return bars.map { $0 / loudest }
    }

    public init(barDuration: TimeInterval, maximumBars: Int) {
    ...
    ///   is already full.
    @discardableResult
    public mutating func add(_ level: Float, at elapsed: TimeInterval) -> Bool {
