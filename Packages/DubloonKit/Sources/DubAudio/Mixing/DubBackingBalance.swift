...
public enum DubBackingBalance {
    ...
    /// a step under.
    public static let originalBedGain: Float = 1
    ...
    public static let bedBelowDialogue: Float = -12
    ...
    public static let fallbackBedGain: Float = 0.75
    ...
    public static let maximumBedGain: Float = 1
    ...
    public static let minimumBedGain: Float = 0.1

    // MARK: - Measuring

    /// How loud a scene's dialogue is, from the film's own recording of each of its lines.
    ///
    ...
    public static func dialogueLevel(ofReferencesAt references: [URL]) -> Float? {
        let levels = references.compactMap { url -> Float? in
            guard let buffer = try? DubAudioLoader.loadVoiceBuffer(from: url, applyFades: false) else { return nil }
    ...
    /// stretch of the scene's timeline a line covers.
    ///
    ...
    public static func bedLevel(of bed: AVAudioPCMBuffer, under stretches: [Range<TimeInterval>]) -> Float? {
        ...
        let levels = stretches.compactMap { stretch -> Float? in
            let first = max(0, Int(stretch.lowerBound * sampleRate))
            let last = min(frames, Int(stretch.upperBound * sampleRate))
    ...
    public static func bedGain(bed: Float?, dialogue: Float?) -> Float {
