...
///
/// Scored against the raw take, before the mix places it on the scene's timeline. The mix
/// forgives a late entry so the finished scene sounds good; the score is where the user is
/// told they were late.
public enum DubScorer {
    ...
    /// Scores the take at `takeURL` against the reference recording at `referenceURL`.
    ///
    /// - Parameters:
    ///   - slug: the line being scored, carried into the result.
    ///   - referenceOnset: where the talking starts inside the reference, in seconds from the
    ///     clip's own start, when it is already known. Measured here when nil.
    ...
    public static func score(
        ...
        referenceURL: URL,
        slug: String,
        referenceOnset: TimeInterval? = nil
    ) -> DubLineScore? {
        ...
        return score(take: take, reference: reference, slug: slug, referenceOnset: referenceOnset)
    }

    /// The measurement itself, on buffers already in `DubAudioLoader.canonicalFormat`.
    public static func score(
        take: AVAudioPCMBuffer,
        reference: AVAudioPCMBuffer,
        slug: String,
        referenceOnset: TimeInterval? = nil
    ) -> DubLineScore? {
        ...
        guard peak(of: take) > 0, peak(of: reference) > 0 else {
            return DubLineScore(slug: slug, timing: 0, pacing: 0, delivery: 0)
        }

        // The original's entry is usually known from import; the take's is measured here.
        // Both are offsets into their own clip, which is what makes them comparable.
        let referenceOnset = referenceOnset ?? firstSpeechFrame(referenceEnvelope).map(seconds) ?? 0
        ...
        return DubLineScore(slug: slug, timing: timing, pacing: pacing, delivery: delivery)
    }
    ...
    public static func timingScore(takeOnset: TimeInterval, referenceOnset: TimeInterval) -> Double {
    ...
    private static func pacingScore(_ take: [Float], _ reference: [Float]) -> Double {
        ...
        let correlation = Correlation.pearson(Array(take.prefix(shared)), Array(reference.prefix(shared)))
    ...
    private static func deliveryScore(_ take: [Float], _ reference: [Float]) -> Double {
        ...
        let shape = Double(max(0, Correlation.pearson(takeLoud, referenceLoud)))
