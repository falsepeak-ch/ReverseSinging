...
nonisolated enum DubTakeCleanup {
    ...
    static let roomBelowSpeech: Float = 0.01
    ...
    static let thresholdAboveFloor: Float = 3
    ...
    /// exactly as it is.
    static let minimumSeparation: Float = 4

    /// Open fast so a consonant is never dulled; close slowly so a word's tail is never cut.
    static let attack: TimeInterval = 0.006
    static let release: TimeInterval = 0.18
    ...
    static func gains(for levels: [Float]) -> [Float] {
    ...
    @discardableResult
    static func apply(to buffer: AVAudioPCMBuffer) -> Bool {
