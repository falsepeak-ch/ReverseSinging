...
nonisolated enum DubAudioLoader {
    ...
    /// Returns one bounded stretch of `buffer`.
    ///
    /// Dialogue alignment needs both edges: keeping everything after the first word also
    /// keeps the take's padded silence, which makes its apparent duration unrelated to the
    /// words that have to fit the actor's mouth movements.
    static func trimming(
        _ buffer: AVAudioPCMBuffer,
        fromOffset startOffset: TimeInterval,
        toOffset endOffset: TimeInterval
    ) -> AVAudioPCMBuffer? {
        let sampleRate = buffer.format.sampleRate
        let startFrame = max(0, min(Int(buffer.frameLength), Int((startOffset * sampleRate).rounded())))
        let endFrame = max(startFrame, min(Int(buffer.frameLength), Int((endOffset * sampleRate).rounded())))
        ...
        for channel in 0..<Int(buffer.format.channelCount) {
            destination[channel].update(from: source[channel].advanced(by: startFrame), count: count)
        }

        trimmed.frameLength = AVAudioFrameCount(count)
        return trimmed
    }

    /// Fits audio to an exact duration without changing its pitch.
    ///
    /// `AVAudioUnitTimePitch` is the system's phase-vocoder. Rendering it offline gives live
    /// playback and export the same already-fitted buffer, with no realtime effect state to
    /// drift between voice lanes.
    static func fittingDuration(
        of buffer: AVAudioPCMBuffer,
        to duration: TimeInterval
    ) -> AVAudioPCMBuffer? {
        let format = buffer.format
        let sourceDuration = Double(buffer.frameLength) / format.sampleRate
        guard sourceDuration > 0, duration > 0 else { return nil }

        let targetFrames = AVAudioFrameCount((duration * format.sampleRate).rounded())
        guard targetFrames > 0 else { return nil }

        // Avoid an effect pass for differences below one frame. Besides being cheaper, this
        // preserves the source sample-for-sample in the normal case.
        if abs(Int64(targetFrames) - Int64(buffer.frameLength)) <= 1 { return buffer }

        // A soundtrack-heavy reference can make the speech detector include an explosion or
        // score tail. Do not turn a one-second interjection into a three-second drawl because
        // of that bad edge; modest corrections fix recording run-up without rewriting the
        // performer's delivery.
        let rate = sourceDuration / duration
        guard (0.75...1.33).contains(rate) else { return buffer }
        ...
        do {
            let quantum: AVAudioFrameCount = 4096
            try engine.enableManualRenderingMode(.offline, format: format, maximumFrameCount: quantum)
            try engine.start()

            player.scheduleBuffer(buffer, at: nil, options: [])
            player.play()

            guard let result = AVAudioPCMBuffer(pcmFormat: format, frameCapacity: targetFrames),
                  let scratch = AVAudioPCMBuffer(pcmFormat: format, frameCapacity: quantum),
                  let resultChannels = result.floatChannelData else { return nil }

            var written = 0
            while written < Int(targetFrames) {
                let request = AVAudioFrameCount(min(Int(quantum), Int(targetFrames) - written))
                let status = try engine.renderOffline(request, to: scratch)

                switch status {
                case .success:
                    guard let scratchChannels = scratch.floatChannelData else { return nil }
                    let rendered = Int(scratch.frameLength)
                    for channel in 0..<Int(format.channelCount) {
                        resultChannels[channel].advanced(by: written)
                            .update(from: scratchChannels[channel], count: rendered)
                    }
                    written += rendered
