...
struct DubBoothCompositingOutputTests {
    ...
    private func writeClip(
        ...
        for frame in 0..<Int(duration * Double(frameRate)) {
            ...
            try #require(appended, "the fixture writer refused frame \(frame): \(String(describing: writer.error))")
        }

        input.markAsFinished()
        await writer.finishWriting()
        try #require(writer.status == .completed, "the fixture clip was not written: \(String(describing: writer.error))")
    }
